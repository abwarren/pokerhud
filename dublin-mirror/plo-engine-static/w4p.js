// W4P Injectable v17-server - PLO Remote Table Control (hero-only: .self-player class ONLY, no fallbacks)
// v17-server: nativeClick + direct fetch (no bridge/extension deps)
...
// Standalone — works without Chrome extension (direct fetch, CORS via Flask)
...
(function(){
  ...
  // ── Direct fetch (standalone — no extension bridge needed) ───
  ...
  function bridgeFetchRaw(path, method, body, callback) {
    var opts = { method: method || 'GET', headers: { 'X-API-Key': API_KEY } };
    if (body) { opts.headers['Content-Type'] = 'application/json'; opts.body = JSON.stringify(body); }
    fetch('https://potlimitomaha.xyz' + path, opts)
      .then(function(r) { return r.json(); })
      .then(function(data) { if (callback) callback({ ok: true, data: data }); })
      .catch(function(e) { console.warn('[W4P] fetch error:', path, e.message); if (callback) callback({ ok: false, error: e.message }); });
  }
  ...
  function detectButtons() {
    ...
    if (heroSeat) {
      _heroActive = heroSeat.classList.contains('active') || !!heroSeat.querySelector('.active-turn');
    }
    ...
    // Gate: only populate result.actions when hero is active
    if (heroSeat && !_heroActive) {
      _lastButtons = result;
      return result;
    }
  ...
  console.log('[W4P] v17-server directFetch+nativeClick | session=' + _sessionId);
